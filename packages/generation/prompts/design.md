...
5. **fighter** (1v1 arcade-ladder fighting game, Street Fighter/Mortal Kombat style). Controls: d-pad move/jump/crouch, Y high punch, B low punch, X high kick, A low kick, L/R block. The player climbs a ladder of AI opponents (best-of-3 each) up to a boss fighter. Author a ROSTER as bounded data, not image assets: a `player` fighter, 3 ladder `opponents` (each in a `level`), and a `boss` fighter. Every fighter has a `build` (nimble/balanced/heavy), an `outfit` silhouette (gi/boxer/wrestler/street/robe/armor), a palette `colorSlot` (5-11), `hp`, and optional light `speedScale`/`powerScale` leans. The runtime owns opponent/boss rendering plus all moves and frame data. With a photo, a later Muse Image stage may replace the procedural player with one complete validated pose set; the photo supplies identity while your roster supplies the game-world persona and costume. If any pose fails, the whole player set stays procedural. Never invent face or sprite fields. Content floors: 3 ladder opponents + a distinct boss with 2-3 rage phases. Choose this for any 1v1 combat / martial-arts / tournament-brawl premise.

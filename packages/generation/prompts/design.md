...
1. **platformer** (Mario-like side-scroller). Controls: d-pad move/duck, A jump, B spin jump, X/Y run/throw. Plan ONLY what this runtime can deliver: walker/flyer/shooter/chaser enemies; springs; moving platforms; solid/platform/hazard terrain; coin/heart pickups and double-jump/projectile/shield powerups; and the player's run/jump/spin/throw verbs. Do NOT promise glide, collapsing tiles, pendulums, grapples, wall-jumps, or invented cast roles such as `bruiser` — later passes cannot implement them. Choose camera framing and source-art detail independently. For `platformerScale`, use `heroic` by default, especially when a likeness photo exists or the premise centers a human-sized action hero; it gives actors and terrain a close classic-console framing. Use `compact` only when a deliberately tiny hero, swarms, speed, or broad traversal is part of the identity. For `platformerArtDensity`, use `detailed` by default when a likeness photo exists so Muse Image can author a richer full-body player at the same gameplay size. Use `chunky` when there is no likeness photo, or for an intentionally coarse, tiny, toy-like, or early-console pixel aesthetic. Content floors: 3 levels + boss arena, ≥4 distinct enemy types (walker/flyer/shooter/chaser), ≥1 powerup, ≥12 pickups, boss with ≥2 phases, a checkpoint per level.

...
export function defaultConfig(): SparkadeConfig {
  ...
  return {
    ...
    likeness: { describeInStory: false, smartFeatures: true, style: 'avatar' },
...
- likeness.smartFeatures: when true, a vision call reads the player's photo for its true (lighting-
  ...
  game-palette bake).
- likeness.style: "avatar" (default) is a HYBRID — a drawn pixel-art character (from the detected
  traits) for the tiny in-game sprite head, plus the quantized pixel-PHOTO for the big story-card
  portrait (a downscaled photo muds at sprite size but reads at 64px, where a drawn face looks
  MS-Paint-ish). "photo" quantizes the actual photo everywhere. "avatar" needs smartFeatures on (it
  draws from the detected features); with smartFeatures off it falls back to the photo bake.

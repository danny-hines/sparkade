...
export function defaultConfig(): SparkadeConfig {
  ...
  return {
    ...
    likeness: {
      ...
      smartFeatures: true,
      style: 'avatar',
      portraitGen: { enabled: false, baseUrl: 'https://api.openai.com/v1', model: 'gpt-image-1', apiKeyEnv: 'OPENAI_API_KEY' },
    },
...
- likeness.portraitGen (EXPERIMENTAL, off by default): when enabled, the large story-card portrait is
  generated by an image model (redraws the photo as a stylised arcade portrait) instead of the pixel-
  photo bake; the tiny in-game sprite head stays the drawn avatar. Placeholder is OpenAI's images/edits
  (set enabled:true + the OPENAI_API_KEY env var). Swap to Meta Muse Image later by changing baseUrl/
  model/apiKeyEnv. Falls back to the photo bake on any failure. Adds latency + per-image cost.

...
- `tileRuns` is the compact tile grid, top to bottom. Each visual row is an array of left-to-right `[tile,count]` tuples, for example `[[".",72],["#",8]]`; expanding adjacent tuples must produce rows of EXACTLY the same width (80–104 chars is the normal target; use up to 120 only when a level's progression truly needs it; hard bounds 32–256) and 14–18 rows tall. One expanded character = one 16px tile. Never output literal `tiles` row strings. `platformerScale: "compact"` shows about 32×18 tiles; `"heroic"` uses a close 2x camera and shows about 16×9. Keep levels TIGHT and dense rather than long and empty — you must finish all three levels well within your time budget.

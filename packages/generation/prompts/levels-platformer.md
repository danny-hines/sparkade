...
- `legend` maps single characters to `solid` (full block), `platform` (one-way, jump through from below), `hazard` (spikes — always place on/above solid ground), or `checkpoint` (respawn lantern — at least one mid-level, standing ON solid ground). `.` is always empty sky. Do NOT author `decoration` or `exit` cells: the engine owns both.

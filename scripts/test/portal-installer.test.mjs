...
if (cmd === 'version') out('Android Debug Bridge version 1.0.41');
  ...
  out('Success');
} else if (cmd === 'shell dumpsys package dev.sparkade.kiosk') out('versionName=0.3.0\\nflags=[ HAS_CODE ' + (cfg.debug ? 'DEBUGGABLE' : '') + ' ]');
else if (cmd === 'shell pm list packages -e ai.wondry.portal') { if (cfg.wondry) out('package:ai.wondry.portal'); }
else if (cmd.includes('SETUP_STATUS')) out('Broadcast completed: result=-1, data="SPARKADE_SETUP_V1;state=' + (cfg.state || 'registered') + ';code=ABCD-2345;version=0.3.0;origin=https://sparkade.dev;"');
else out('OK');
`;

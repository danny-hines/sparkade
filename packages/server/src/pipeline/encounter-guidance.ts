...
export function encounterGuidance(
  ...
  const recentPlatformers = recent
    .filter((g) => g.archetype === 'platformer')
    .slice(0, GENERATION.archetypeHistoryGames);
  return [
    ...
    ...(recentPlatformers.length
      ? [
          `RECENT DELIVERED ENCOUNTERS: ${JSON.stringify(recentPlatformers.map((g) => g.encounters.filter((s) => /^(pattern|variant|modifier):/.test(s))))}. Prefer less-used variants and different ordered sequences; explicit player requests still take precedence.`,
        ]
      : []),
  ].join('\n\n');
}

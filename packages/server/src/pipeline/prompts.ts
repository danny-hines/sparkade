...
function spriteMenu(archetype: ArchetypeId): { libList: string; reskinNotes: string } {
  ...
  const extraRoles: Record<ArchetypeId, string> = {
    ...
    racing:
      'Nothing to reskin — racing vehicles come from the generated image pack, not sprites.assign. Keep the hero/boss placeholder assigns; the design racingIdentity (pilot, world/conveyance/rider concepts, composable traversal, sound profile, boost supply) already carries the identity for this cup, so author no new identity here. Put per-race character into circuit names, rival pace, theme accents, and world dressing (envConcept + track materials).',
  };
